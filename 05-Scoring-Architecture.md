...
| type | payload | notes |
|---|---|---|
| `game_start` | `{opponent, lineup: [playerId,…], innings, scheduleGameId?, isHome?, mercyLimit?}` | Creates the game; publishes the lineup so other devices inherit it. `scheduleGameId` links to the manifest schedule fixture *(added 2026-07-11, see `07-Portal-Architecture.md`)*; absent for unscheduled games. `isHome` *(added 2026-07-12, closing the home/away gap — see the callout below the table)*: `true` = Pickles kicks the **bottom** of each inning (home); absent/`false` = we lead off (away), which replays identically to the pre-`isHome` engine, so legacy events are unaffected. |
| `pa` | `{playerId, result, inning, half}` | result ∈ `1B 2B 3B HR FLY GND K MISSED SKIP` (league has no BBs, no errors-as-results; K exists). *(An `FC` result was added and deprecated within the 2026-07-14 session — see the fielder's-choice bullet under Assumption engine.)* `MISSED` = PA happened, unseen. `SKIP` = no PA, pointer advance only — excluded from PA counts entirely |
...
| `adjust` | `{field: "outs"\|"score_us"\|"score_them"\|"inning"\|"half"\|"isHome"\|"mercyLimit", value}` | Score-bug direct correction; the app never blocks on inconsistency. `half` *(added 2026-07-12 in-template; folded in with Jason's sign-off 2026-07-13)*: the adjust sheet's inning stepper walks both inning **and** half (▲3 → ▼3 → ▲4), which the original four-field enum couldn't record — same one-adjust-event-per-changed-field draft/commit convention as the rest of the sheet. `isHome` *(added 2026-07-12)*: the post-start correction path for a coin toss that lands after `game_start` committed — flips which side leads off from that point of the replay forward; deliberately does **not** swap the current half (the `half` field above handles that if it's also wrong). Replay order within one commit: `isHome` first, since it changes how any half/inning value in the same commit reads |
| `amend` | `{target_event_id, result}` | Fix-last **and the scorecard editor** *(2026-07-14)*: replay uses the amended result — any past PA of the current game, not just the newest |
| `undo` | `{target_event_id}` | Tombstone; replay skips target. Targeting an `insert`'s own id suppresses its splice entirely; targeting the wrapped event's id tombstones just it |
| `insert` | `{after_event_id, event: {id, ts, type, payload}}` | *(Added 2026-07-14 — the scorecard editor's retro primitive.)* Meta-event like amend/undo: appended with a normal seq (log stays append-only, sync rows never renumber — invariant 18b), but replay splices the wrapped event into the **effective order** right after `after_event_id` (`effectiveScoringEvents`, deriveState pass 0). The wrapped event carries its own id, so it can itself be amended, undone, or anchor a later insert. Every `eventIdx` the engine stamps indexes the effective list — consumers read `st.effectiveEvents`, never the raw array |
...
- **Fielder's choice: logged 1B + runner-out, now by principle rather than simplification.** *(History: the original text here called this an "accepted simplification." Game 2 (2026-07-13) produced the play — Mike's kick fielded to the left side, Eli tagged advancing — and the 2026-07-14 session briefly built a first-class `FC` result (AB-not-hit). Jason deprecated it the same session, before it ever shipped, on kickball grounds: the throw to 1st almost never beats the kicker, so a fielder prioritizing another out doesn't negate the single — the kicker earned the hit. `FC` is not in the result enum; the pad carries no button; the convention of record is the kicker's true result plus a runner-out on whoever was tagged.)*
- **Mercy limit is per-game config (2026-07-14, game-2 field lesson — the hardcoded 8 never fired against a real 6-run rule):** `game_start.payload.mercyLimit` (set by a stepper on the Start Game fixture card, defaulting from manifest `scoring.mercyRunLimit`) > manifest > legacy 8, correctable mid-game via `adjust {field:'mercyLimit'}` on the adjust sheet. The banner stays a choice, never an automatic cutoff; final inning stays exempt.
- **The adjust sheet opens from BOTH halves (2026-07-14):** the score bug is tappable on the defense screen too (game 2's phantom-runs hack happened exactly because it wasn't), flips into the cancel control while its sheet is open (tap-again-to-cancel), and carries an immediate **"end half-inning now"** action — one tap commits `adjust {outs:3}` with full turnover side-effects, the app keeping pace with the game; score fixes come after, calmly.
- **Scorecard editor (2026-07-14, Jason's game-2 field report):** past at-bats are correctable from the box score — tap a snake-grid cell to amend its result, restore a wrongly-erased runner ("wasn't out — back on base"), log a missed basepath out (candidates derived by prefix replay), insert a missed at-bat before/after, remove one (two-tap confirm), or end the half after it; tap an opponent line-score cell for a runs stepper (decrements tombstone the newest `opp_run`s of that inning — an appended −1 would fight later absolute `score_them` adjusts; increments append inning-tagged runs the engine keeps out of the live half's mercy tally). All commits are appended events; replay re-derives the card. Scope: current game only. Accepted caveat: retro-inserting/removing an at-bat shifts the live lineup pointer by one — visible on NOW UP, fixed by a manual tap or SKIP.
