...
| `adjust` | `{field: "outs"\|"score_us"\|"score_them"\|"inning"\|"isHome", value}` | Score-bug direct correction; the app never blocks on inconsistency. `isHome` *(added 2026-07-12)*: the post-start correction path for a coin toss that lands after `game_start` committed — flips which side leads off from that point of the replay forward; deliberately does **not** swap the current half (the same sheet's inning stepper handles that if it's also wrong). *(A 5th value, `"half"`, is implemented in-template with inline rationale but still awaits fold-in sign-off — see `_ai/handoff.md`.)* |
...
- `1B`: kicker→1st, every runner +1 base. `2B`: kicker→2nd, runners +2. `3B`: kicker→3rd, runners +3 (i.e. all score). `HR`: everyone scores.
