...
| Team-phone boot (corrected) | Team phone never sees the *classic screens* pre-game — `decidePortalBoot()` always calls Start Game directly, regardless of `scheduleGameInWindowNow()`. `#tabbar` stays hidden until a game is actually live. | **Corrected 2026-07-12** — Jason, on-device: the original window-gated version fell through to "the old card-style lineup page" outside a game window, and made the PIN entry point unreachable without first building a classic lineup. Once a game *is* live, the tab bar returns as the firm-principle-5 manual-override fallback, same as any device. *(Amended same day, "portal as neutral ground" row below: the original "never sees the portal / PIN sheet has no cancel at all" stance was loosened — the read-only portal became the team phone's legitimate stats-and-sounds surface. Scoring remains hard-locked behind the PIN.)* |
| Portal as neutral ground | Leaving any **pre-game** scoring surface lands in the portal, on every device: (1) the team-phone PIN sheet carries a dashed "‹ just the stats & sounds" link into the portal (its player-row play buttons ARE the team phone's no-PIN walk-up-sounds access; scoring stays PIN-locked); (2) the Start Game editor's ✕ cancel goes to the portal (fixes both audit strandings — team phone falling to classic screens, prompt-accepting teammates marooned on operator surfaces); (3) the portal header shows a **✎ score** entry on the stats/schedule screens for the team phone + PIN-unlocked devices (rule 3, "a tap or two away at all times," now actually built — also the team phone's route *back* to Start Game); (4) **ending a game lands in the portal** on every device (postgame stats + play-button hype on the team phone). Mechanically: the game-window prompt's accept no longer clears `portalActive` — `openStartGameFlow`'s proceed() does, once the editor truly opens — and the PIN sheet renders ahead of the portal shell in `render()`. Boot posture unchanged: a fresh team-phone open still goes straight to Start Game. | **Signed off + built 2026-07-12** (second round, same day as the audit). Tests: group 69. |
