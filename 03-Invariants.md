...
**10c. The viewport meta tag carries only `width=device-width, initial-scale=1.0, viewport-fit=cover` — no `maximum-scale`/`user-scalable=no`.**
Those two used to be there for the obvious reason (block pinch-zoom/double-tap-zoom on a kiosk soundboard), but invariant 9 already established iOS has ignored `user-scalable=no` as a meta directive since iOS 10 — `touch-action: manipulation` (CSS) is what's actually doing that job, so dropping them from the meta tag changes nothing about zoom behavior. What it DOES fix: `maximum-scale`/`user-scalable=no` alongside `viewport-fit=cover` is a documented iOS bug that throws off `env(safe-area-inset-*)` — this is what caused the "fairly significant" gap between the tab bar and the home indicator Jason found 2026-07-14 on both an XS Max and a 15 Pro Max, both standalone home-screen installs. `#app`'s top/bottom insets also got a `clamp(12px, env(...), 40px)` ceiling (was unbounded `max()`) as a belt-and-suspenders guard, but the meta tag was the actual root cause. Don't re-add `maximum-scale`/`user-scalable=no` to "be safe" — it's not free, it's this bug.
