...
**If #3 also doesn't close it**, the next, more forceful step (not yet tried, skipped so far because it's more invasive) is dropping CSS viewport units/insets entirely for the outer box and instead measuring the real viewport in JS (`window.visualViewport.height`, with a `resize`/`visualViewport.resize` listener) into a CSS custom property (`--real-vh`), and sizing `#app`/`body` off that instead of any CSS-only viewport unit. That's the guaranteed-correct fallback if CSS-only approaches keep proving unreliable on these specific devices/iOS builds — more code, but no more guessing about which viewport unit iOS actually honors this week.
