...
5. **[BUILT 2026-07-13, attempt #5 + instrumentation]** In standalone mode with `viewport-fit=cover` the web view spans the entire physical screen by definition, so `screen.height` (portrait-guarded) joins the candidate set and `--real-vh` takes the **max** of screen.height / visualViewport / innerHeight when standalone; Safari-tab mode still excludes screen.height (browser chrome). Crucially, the **debug readout now prints every source** (`vp[SA]: sh932 vv… ih… ce… →N`), so one wordmark-tap screenshot finally shows which number iOS lies about. If the gap STILL survives with `→932`-style output (i.e. the height is right but space remains), the suspect list flips from viewport height to: env(safe-area-inset-bottom) over-reporting inside `#app`'s clamp (diagnostic: hard-code `bottom: 12px`), or `.tabbar`'s own margin.
