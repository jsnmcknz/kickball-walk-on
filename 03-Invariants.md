...
**10c. [OPEN, 2026-07-12] Bottom gap between the tab bar and the home indicator — under active investigation, not yet resolved.**
On both an XS Max and a 15 Pro Max (standalone home-screen installs), a "fairly significant" static gap shows up below `.tabbar`, well past what `#app`'s bottom inset should produce. Tried so far, in order, NONE confirmed as the fix yet:
...
4. **[BUILT 2026-07-13, attempt #4 — Jason re-opened the parked issue after #3 failed on-device]** The forceful step is now in: `syncRealViewportHeight()` measures the live viewport in JS (`window.visualViewport.height`, `innerHeight` fallback) into `--real-vh` on every resize/orientation/pageshow plus 300ms/1000ms boot settle timers; `body { height: var(--real-vh, 100%) }` (fallback deliberately `100%`, NOT `dvh` — an invalid-at-computed-value var fallback would nuke the earlier height declarations instead of yielding to them); and **`#app` switched `position: fixed` → `absolute`** so its bottom inset resolves against body's JS-corrected box rather than the stale viewport that fixed positioning anchors to. If confirmed on-device, this entry graduates from OPEN to the invariant "the outer box trusts JS measurement, never CSS viewport units — do not 'simplify' back to fixed/dvh." If a gap STILL shows, the remaining suspects are env(safe-area-inset-bottom) over-reporting inside the clamp (try a fixed 12px bottom inset as a diagnostic) or the tab bar's own bottom margin.
