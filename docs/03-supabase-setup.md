...
- **Belt-and-suspenders:** set up a pinger that requests `https://YOUR-PROJECT.supabase.co/rest/v1/games?select=id&limit=1` once a day with one custom header: `apikey`, set to your client key from step 3. *(Corrected 2026-07-13: an earlier version of this doc said to also send `Authorization: Bearer …` — that works for legacy anon keys, which are JWTs, but a new-style publishable key in that header makes every request fail with HTTP 401, because the server tries to decode any Bearer value as a JWT. The `apikey` header alone is correct for both key kinds. The app itself learned this the same morning.)* Daily traffic = never pauses. **Easiest for Jason: a Make.com scheduled scenario** (one HTTP "Make a request" module, daily schedule — ~30 ops/month, negligible; same pattern as the Henderson pipelines). cron-job.org's free tier is the no-Make alternative. Daily beats weekly here: a mid-week-only ping leaves no slack if a run fails or a game-morning ritual gets skipped.
